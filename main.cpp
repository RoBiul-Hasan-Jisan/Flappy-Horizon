...
void drawDeveloperScreen() {
    ...
    y -= 22;
    drawText(cx - 125, y, "Special Thanks & Guidance", 0.7f, 0.8f, 0.5f, 12);
...
void display() {
    ...
    if (gameStarted && gameRunning) {
        ...
        if (currentLevel == 1) {
            drawText(SCREEN_WIDTH/2 - 80, SCREEN_HEIGHT - 110, "STATIC PIPES", 0.5f, 0.8f, 0.5f, 10);
        } else if (currentLevel == 2) {
            drawText(SCREEN_WIDTH/2 - 100, SCREEN_HEIGHT - 110, "SLOW MOVEMENT", 0.7f, 0.9f, 0.5f, 10);
        } else if (currentLevel == 3 && !pipesFlipped) {
            drawText(SCREEN_WIDTH/2 - 100, SCREEN_HEIGHT/2, "NEXT: PIPES WILL FLIP!", 1, 0.5f, 0.2f, 14);
        }

        if (pipesFlipped) {
            drawText(SCREEN_WIDTH - 140, SCREEN_HEIGHT - 55, "FLIPPED MODE!", 1, 0.3f, 0.1f, 12);
        }

        float speedPercent = (currentPipeSpeed / MAX_PIPE_SPEED) * 100;
        char speedText[50];
        sprintf(speedText, "Speed: %.0f%%", speedPercent);
        drawText(SCREEN_WIDTH - 120, SCREEN_HEIGHT - 80, speedText, 0.8f, 0.8f, 1.0f, 12);

        char nextSpeedText[50];
        sprintf(nextSpeedText, "Next +Speed: %d pts", nextSpeedScore);
        drawText(SCREEN_WIDTH - 180, SCREEN_HEIGHT - 105, nextSpeedText, 0.7f, 0.7f, 0.9f, 10);

        drawText(SCREEN_WIDTH - 160, SCREEN_HEIGHT - 130, "BEST: " + to_string(highScore), 1, 1, 0, 12);

        float moveAmp = getCurrentMoveAmplitude();
        if (moveAmp == 0) {
            drawText(12, SCREEN_HEIGHT - 55, "Pipes: Static", 0.5f, 0.7f, 0.5f, 10);
        } else if (moveAmp < 30) {
            drawText(12, SCREEN_HEIGHT - 55, "Pipes: Slow Move", 0.5f, 0.8f, 0.5f, 10);
        } else {
            drawText(12, SCREEN_HEIGHT - 55, "Pipes: Full Move", 0.5f, 0.9f, 0.5f, 10);
        }
        drawText(12, SCREEN_HEIGHT - 70, "Gap: Fixed (180px)", 0.5f, 0.7f, 0.5f, 10);
    }

    drawText(12, SCREEN_HEIGHT - 30, getTimeLabel(), 1, 1, 0.7f, 12);
